...
namespace Client
{
    public class ZalbaClient : ChannelFactory<IZalbaService>, IZalbaService, IDisposable,IZalba
    {
        private IZalbaService factory;
        private X509Certificate2 clientCertificate;

        public ZalbaClient(NetTcpBinding binding, EndpointAddress address) : base(binding, address)
        {
            try
            {
        ...
        public string TestConnection()
        {
            try
            {
                ...
                //Console.WriteLine("[CLIENT] Test connection successful");
                return "[CLIENT] Test connection successful";
            }
            catch (Exception ex)
            {
